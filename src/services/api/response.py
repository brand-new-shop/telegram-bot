...
def raise_for_error(status_code: int, response_body: dict) -> None:
    detail = response_body.get('detail', f'Unexpected status code "{status_code}"')
    error_detail_to_exception_class = status_code_to_errors.get(status_code, {})
    exception_class = error_detail_to_exception_class.get(detail, exceptions.ServerAPIError)
    raise exception_class(detail)


def get_decoded_json_and_check_for_errors(response: httpx.Response) -> list | dict:
